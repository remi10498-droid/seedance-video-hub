...
// Строгие параметры под каждую модель
const MODEL_SPECS = {
  "seedance-2.5": {
  ...
  "seedance-2.0": {
    ...
    resolutions: [{ id: "480p", label: "480p" }, { id: "720p", label: "720p (HD)" }, { id: "1080p", label: "1080p (FHD)" }, { id: "4k", label: "4K (Ultra HD)" }],
    ratios: ["16:9", "9:16", "1:1", "4:3", "3:4", "21:9"],
    hasAudio: true,
  },
  "flux-3-video": {
...
// Единый постоянный ключ для защиты от потери картинок
...
export default function MediaStudio() {
  ...
  // Предварительный расчёт стоимости для фронтенда
  useEffect(() => {
    ...
    pollTimerRef.current = setInterval(async () => {
      ...
      try {
        ...
        if (data.status === "DONE" && data.url) {
          ...
          // Считываем РЕАЛЬНОЕ разрешение и РЕАЛЬНУЮ длину
          const tempVideo = document.createElement("video");
          tempVideo.src = data.url;
          tempVideo.onloadedmetadata = () => {
            const actualSeconds = Math.round(tempVideo.duration) || itemMeta.duration;
            const actualResolution = tempVideo.videoWidth ? `${tempVideo.videoWidth}×${tempVideo.videoHeight}` : itemMeta.resolution;
            
            // Берем реальные кредиты и модель от сервера (если не отдал, берем наши изначальные)
          ...
          // Fallback если браузер не смог загрузить метаданные
          tempVideo.onerror = () => {
            const newItem = {
              id: taskId || Date.now().toString(),
              url: data.url,
  ...
  const handleGenerate = async (e) => {
    ...
    if (startFrameUrl) {
        formData.append("start_frame", startFrameUrl);
        formData.append("image_url", startFrameUrl);
    }
    ...
    try {
      ...
      if (taskId) {
        pollStatus(taskId, { prompt, model, duration: Number(duration), resolution, cost });
      } else {
  ...
  return (
    <main style={{ maxWidth: "860px", margin: "30px auto", padding: "24px", fontFamily: "sans-serif", background: "#111", color: "#fff", borderRadius: "12px" }}>
      ...
      <form onSubmit={handleGenerate} style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
        <div>
          <label style={{ fontSize: "12px", color: "#aaa" }}>Код доступа (сохраняется автоматически):</label>
          <input
            type="password"
            placeholder="SEED480"
            value={accessCode}
            onChange={(e) => { setAccessCode(e.target.value); localStorage.setItem("ai_access_password", e.target.value); }}
      ...
      <div style={{ marginTop: "30px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid #222", paddingBottom: "10px", marginBottom: "16px" }}>
          ...
          {history.length > 0 && (
            <button onClick={() => { setHistory([]); localStorage.removeItem(STORAGE_KEY); }} style={{ background: "transparent", border: "none", color: "#888", fontSize: "12px", cursor: "pointer", textDecoration: "underline" }}>
              Очистить историю
            </button>
          )}
        </div>
            ...
            {history.map((item) => (
              ...
              >
                <div style={{ width: "100%", height: "140px", background: "#000", position: "relative" }}>
                  ...
                  {/* Компактные бейджи с РЕАЛЬНОЙ информацией от API Picsart */}
